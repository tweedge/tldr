...
def write_to_json(filename, json_serializable_dict):
    ...
    file_handler.write(json.dumps(json_serializable_dict))
    file_handler.close()
...
def get_dig_axfr_output(hostname, nameserver):
    try:
        ...
        record = f'TLDR_DIG_FAILED [{str(error.returncode)}] {error.stderr.decode("utf-8").strip()}'
        return record


def zone_transfer_succeeded(zone_data):
    if "TLDR_DIG_FAILED" in zone_data:
        return False
...
if __name__ == "__main__":
    ...
    for root_ns in ROOT_NAMESERVER_LIST:
        ...
        if zone_transfer_succeeded(zone_data):
            zone_transfer_enabled_list.append({"nameserver": root_ns, "hostname": "."})
    ...
    for tld in tlds:
        ...
        nameservers = dnstool.get_nameserver_list(full_tld)
    ...
    for zone_status in zone_transfer_enabled_list:
        if zone_status["hostname"] == ".":
            zone_transfer_enabled_markdown += (
                ...
                + "` via `"
                + zone_status["nameserver"]
                + "`: [Click here to view zone data.]("
                + "archives/root/"
                + zone_status["nameserver"]
                + "zone)\n"
            )
